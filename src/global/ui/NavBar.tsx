...
import { observeApi, observeAuthorList, observeBlogMenu, observeEditor, observeRepo } from '../../blog/BlogContext'
...
export const NavBar = observer(() => {
  ...
  return <HStack fontSize='0.9rem' position='fixed'
                 top='0' left='0'
                 bgColor={editor.editMode ? theme.appBg : undefined}
    ...
    {selectedBook && api.isServerRunning && <>
      ...
      <Switcher isSelected={editor.editMode}
...
const RedButton = (props: ButtonProps) => {
  ...
  return <Button textColor='#ffFFff'
...
const ToolsPanel = observer(({ book }: { book: Book }) => {
  ...
  const createPage = () => {
    if (editor.editMode) {
      if (editor.selectedPage) {
        const curPageIndex = book.pages.findIndex(p => p.uid === editor.selectedPage?.uid)
        editor.selectedPage = (book.createPage(curPageIndex + 1))
      } else {
        editor.selectedPage = book.createPage(book.pages.length)
  ...
  const movePageUp = () => {
    if (editor.editMode && editor.selectedPage) {
      editor.selectedPage?.book?.movePageUp(editor.selectedPage)
    }
  }

  const movePageDown = () => {
    if (editor.editMode && editor.selectedPage?.book) {
      editor.selectedPage?.book.movePageDown(editor.selectedPage)
    }
  }
  ...
  const deletePage = () => {
    if (editor.editMode && editor.selectedPage) {
      ...
      editor.selectedPage?.book?.remove(editor.selectedPage)
      editor.selectedPage = undefined
    }
  }
